...
def parse_html(html, keyword):
    ...
    for card in cards:
        try:
            ...
            # Based on the example: bidsinfo.com/ref/[REF_NO]/[SLUG]
...
def run_scraper(user_keywords, update_source="Manual", progress_callback=None):
    ...
    for keyword in all_keywords:
        for page in range(1, PAGES_TO_SCAN + 1):
            ...
            # Add update source to each record
            for item in data:
                item["Update Source"] = update_source

            all_data.extend(data)
            current_step += 1
            if progress_callback: progress_callback(current_step, total_steps, keyword, page)
            time.sleep(0.3)
